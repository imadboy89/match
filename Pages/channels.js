...
class ChannelsScreen extends React.Component {
  ...
  onchannel_clicked =(item)=>{
    ...
    if(item.channel_servers!=undefined){
      Linking.openURL(item.channel_servers[0].SecureUrl);
    }
    if(item.category_id!=undefined && item.channel_id==undefined){
      ...
      API_.get_channel(item.channel_id).then(resp=>{
        if(resp["data"]["name"] != undefined){
          ...
          this.channel_servers = data["channel_servers"];
          let url =this.channel_servers[0].SecureUrl;
          //Linking.openURL(this.channel_servers[0].SecureUrl);

          API_.saveLink(url,resp["data"]["name"],API_.domain_o+item.channel_photo);
          //this.setState({modalVisible_match:true,url:url});
